...
async function launchBrowser() {
    const launchOptions = {
        headless: true,
        args: ['--no-sandbox', '--disable-setuid-sandbox', '--single-process', '--disable-gpu', '--disable-dev-shm-usage']
    };
    const envPath = process.env.CHROME_PATH || process.env.PUPPETEER_EXECUTABLE_PATH || process.env.CHROMIUM_PATH;
    if (envPath) {
        return await puppeteer.launch({ ...launchOptions, executablePath: envPath });
    }
    try {
        return await puppeteer.launch(launchOptions);
    } catch (err) {
        console.warn('[BROWSER] Puppeteer launch failed:', err.message);
...
//  CONFIG
// ============================================================
const BOT_TOKEN    = process.env.BOT_TOKEN || "8906099266:AAEPciNvOPzpLS0RZmiw8-H8RjXtJlYMzs8";
const OWNER_ID     = process.env.OWNER_ID ? Number(process.env.OWNER_ID) : 1865939951;
const OWNER_PASS   = process.env.OWNER_PASS || "123";
const ADMIN_HANDLE = process.env.ADMIN_HANDLE || "@lucifer1570";
const REG_LINK     = process.env.REG_LINK || "https://bdgwinuu.com/#/register?invitationCode=7442815992780";
const WIN_STICKER  = process.env.WIN_STICKER || "CAACAgUAAxkBAAFHUGNp4JX1-ohP4uBEWpfNptaz-HmwVgAC4hgAAhboKVbObuGuTcMs2zsE";
const LOSS_STICKER = process.env.LOSS_STICKER || "CAACAgUAAxkBAAFHUGVp4JX-BE2TRkhIKTwcjkwW-gzdPAACthoAAoG8YVYiydObSa0O8zsE";
    ...
    if (wasWin) {
        ...
        if (st.level === 4) {
            ...
            if (dangerousPatterns.includes(last4)) {
                ...
                console.log(`[USER ${userId}] Dangerous Pattern ${last4} at L4 Entry -> Forcing skip of ${state.skipCount} cycles.`);
            }
        }
    }
}
...
async function runPredict(userId, chatId) {
    ...
    if (!cfg.enabled) {
        ...
        console.log(`[USER ${userId}] Skip cycle active: ${state.skipCount} remaining, current level L${st.level}`);
        skippedCycle = true;
        state.skipCount--;
        canBet = false;
...
function startBot(){
    if (!BOT_TOKEN) {
        console.error("❌ BOT_TOKEN not set. Set environment variable BOT_TOKEN and restart the process.");
        return;
    }
